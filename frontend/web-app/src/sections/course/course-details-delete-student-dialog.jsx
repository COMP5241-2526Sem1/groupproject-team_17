...
export default function CourseDetailsDeleteDialog({
  ...
  const handleConfirm = async () => {
    ...
    var res = await ClassManagementActions.removeStudentsFromCourse(courseId, students.map(student => student.id));
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Iconify icon="solar:trash-bin-trash-bold" sx={{ color: 'error.main', width: 24, height: 24 }} />
      ...
      <DialogContent>
        <Alert severity="warning" sx={{ mb: 2 }}>
          This action cannot be undone. The selected student{isMultiple ? 's' : ''} will be removed from this course.
        </Alert>
        <Typography sx={{ mb: 2 }}>
        ...
        <List dense sx={{ maxHeight: 200, overflow: 'auto', border: 1, borderColor: 'divider', borderRadius: 1 }}>
          {students.map((student, idx) => (
            <ListItem key={student.id || student.studentId || idx} divider={idx < students.length - 1}>
              <ListItemText
